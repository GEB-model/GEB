...
*   Identifying waterbody locations and their outflow points, as well as retention basins.
...
*   **Waterbody Outflows**: Water released from reservoirs, lakes, and retention basins.
...
*   **Retention Basin Inflow**: The volume of water entering retention basins.
...
1. **Sideflow Calculation**: The hourly runoff depth is converted to volume (m³) by multiplying by the cell area.
...
3. **Routing Execution**: The routing algorithm (Kinematic Wave or Accuflux) is executed with this hourly sideflow, along with other inputs like evaporation and waterbody/retention basin outflows. This updates the river discharge and storage for that hour.
...
## Interaction with Retention Basins (!!!! The retention logic is currently under construction !!!!)

We differentiate between two operating systems of retention basins: controlled and uncontrolled. Not always is information on the operating type available. Then, we assume the basin is uncontrolled. When water reaches a river cell connected to an uncontrolled retention basin, it starts filling until the maximum storage capacity is reached. Since operating rules for controlled basins are not widely available, we assume they are operated to 'shave' the peak of the flood. A threshold discharge is then used to activate the basin. Once activated, it fills until the maximum storage capacity is reached.    

1. **Inflow (uncontrolled)**: When a river cell is connected to a retention node (uncontrolled basin), the basin starts to fill until the maximum storage capacity (m³) is reached.
2. **Inflow (controlled)**: When a river cell is connected to a retention node (controlled basin), the basin starts to fill when the threshold discharge in the upstream river cell is reached. Inflow occurrs until the maximum storage capacity (m³) is reached. 
3. **Outflow**: The cross section of outflow structures is not widely available. We therefore assume the retention basin releases 1% of its stored water in each timestep (h). 
4. **Storage**: The total inflow is diverted from the upstream river cell and storage is updated every timestep (h) in the model. When the maximum storage capacity of each basin is reached, the basin receives no more inflow.  
