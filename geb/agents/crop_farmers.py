...
def adjust_irrigation_to_limit(
    ...
    irrigation_limit_adjustment: float,
...
def abstract_water(
    ...
    irrigation_limit_adjustment: np.ndarray,
    ...
    # tests
    potential_irrigation_consumption_farmer_m3_test = np.zeros(
        activation_order.size, dtype=np.float32
    )
    ...
    for activated_farmer_index in range(activation_order.size):
        ...
        if farmer_has_access_to_irrigation_water:
            ...
            potential_irrigation_consumption_farmer_m3_test[farmer] = (
                potential_irrigation_consumption_farmer_m3
            )
            ...
            if potential_irrigation_consumption_farmer_m3 > 0.0:
                ...
                if not np.isnan(remaining_irrigation_limit_m3[farmer]):
                    potential_irrigation_consumption_m = adjust_irrigation_to_limit(
                        ...
                        irrigation_limit_adjustment=irrigation_limit_adjustment,
    ...
    return (
        ...
        returnFlowIrr_m,
        addtoevapotrans_m,
        potential_irrigation_consumption_farmer_m3_test,
    )
...
class CropFarmers(AgentBaseClass):
    ...
    def __init__(self, model, agents, reduncancy: float) -> None:
        ...
        self.water_price_full = load_economic_data(
            self.model.files["dict"]["economics/water_price"]
        )
        self.diversions_total = load_economic_data(
            self.model.files["dict"]["economics/diversions"]
        )
        ...
        self.irrigation_moment = self.model.config["agent_settings"]["calibration"][
            "irrigation_moment"
        ]
    ...
    def initiate(self) -> None:
        ...
        self.potential_irrigation_consumption_farmer_m3_test = AgentArray(
            n=self.n, max_n=self.max_n, dtype=np.float32, fill_value=0
        )
        ...
        eff_nsw = self.model.config["agent_settings"]["farmers"]["expected_utility"][
            "adaptation_sprinkler"
        ]["eff_nsw"]
        ...
        self.farmer_class_water = AgentArray(
            n=self.n, max_n=self.max_n, dtype=np.int32, fill_value=-1
        )

        # 0 is 50% efficiency, 1 is 70% and 2 is 90%
    ...
    def determine_water_price_fixed(self):
        water_price = np.full(
            ...
            self.get_value_per_farmer_from_region_id(
                self.water_price_full, self.model.current_time
            ),
            dtype=np.float32,
        )  # $ / ML
        return water_price / 1000  # $ / m3

    def set_irrigation_limit(self):
        diversions_total = np.full(
            self.n,
            self.get_value_per_farmer_from_region_id(
                self.diversions_total, self.model.current_time
            ),
            dtype=np.float32,
        )
        ...
        allocation_vic = diversions_total[aus_region_agents == 0][0] * 1000
        allocation_nsw = diversions_total[aus_region_agents == 1][0] * 1000
        ...
        pasture_allocation_vic = (
            max(
                allocation_vic - crop_allocation_vic - np.sum(livestock_demand_vic),
                0,
            )
            / 365
        ...
        pasture_allocation_nsw = (
            max(
                allocation_nsw - crop_allocation_nsw - np.sum(livestock_demand_nsw),
                0,
            )
            / 365
        )  # daily water demand in m3
    ...
    def abstract_water(
        ...
        It's main purpose is to call the relevant numba function to do the actual abstraction.
        ...
        (
            ...
            returnFlowIrr_m,
            addtoevapotrans_m,
            self.potential_irrigation_consumption_farmer_m3_test[:],
        ) = abstract_water(
            ...
            self.irrigation_limit_adjustment,
            ...
            calibration_irrigation_moment=self.irrigation_moment,
        )
    ...
    def calculate_yield_spei_relation_test_solo(self):
        ...
        for agent_idx in range(n_agents):
            ...
            plt.title(
                f"Agent {agent_idx}, irr class {self.farmer_class_water[agent_idx]}, crop {self.crop_calendar[agent_idx, 0, 0]} "
            )
            plt.legend()
            plt.grid(True)
    ...
    def calculate_yield_spei_relation_test_group(self):
        ...
        crop_elevation_group = self.create_unique_groups(self.farmer_class_water, 10)
    ...
    def adapt_crops(self) -> None:
        ...
        # Make channel and groundwater into the same class
        farmer_class = self.farmer_class_water.copy()
        farmer_class[farmer_class == 2] = 0
        ...
        (
            ...
            new_crop_nr,
            new_farmer_id,
        ) = self.profits_SEUT_crops(farmer_class, unique_crop_calendars)
    ...
    def adapt_irrigation_well(
        ...
        (
            ...
            self.farmer_class_water, adapted, energy_cost, water_cost
        )
        ...
        (
            ...
            total_profits_adaptation,
            profits_no_event_adaptation,
        ) = self.profits_SEUT(self.farmer_class_water, adaptation_type, adapted)
    ...
    def calculate_water_costs(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        operation_cost_sprinkler = (
            ...
            * self.price_adjustment_sprinkler
        )
        operation_cost_drip = (
            np.full(
                self.n,
        ...
        water_cost_m3 = self.water_price  # $ / m3
        ...
        crop_elevation_group = self.create_unique_groups(self.farmer_class_water)
        ...
        mask_reservoir = self.farmer_class_water == 1
        ...
        water_costs[mask_channel] = (
            average_extraction[mask_channel] * water_cost_m3[mask_channel]
        )

        # Compute water costs for agents using reservoir water (LCU/year)
        water_costs[mask_reservoir] = (
            average_extraction[mask_reservoir] * water_cost_m3[mask_reservoir]
        )

        # Compute water costs for agents using groundwater (LCU/year)
        water_costs[mask_groundwater] = (
            average_extraction[mask_groundwater] * water_cost_m3[mask_groundwater]
        )
    ...
    def profits_SEUT(
        self, farmer_class, adaptation_type: int, adapted: np.ndarray = None
    ) -> Union[
        Tuple[np.ndarray, np.ndarray],
        Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray],
    ]:
        ...
        if adaptation_type != 0:
            ...
            gains_adaptation = self.adaptation_yield_ratio_difference(
                farmer_class, adapted, yield_ratios
            )
    ...
    def profits_SEUT_crops(
        self, farmer_class, unique_crop_calendars
    ) -> Tuple[
        np.ndarray,
        np.ndarray,
        np.ndarray,
    ...
    def adaptation_yield_ratio_difference(
        self,
        farmer_class: np.ndarray,
        adapted: np.ndarray,
        yield_ratios: np.ndarray,
    ) -> np.ndarray:
        """
        ...
        crop_elevation_group = np.hstack((crop_elevation_group, adapted.reshape(-1, 1)))
    ...
    def adaptation_water_cost_difference(
        self, farmer_class: np.ndarray, adapted: np.ndarray, energy_cost, water_cost
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Calculate the water cost difference between agents that have adapted and those that have not.
    ...
    def step(self) -> None:
        ...
        if self.model.current_time.month == 7 and self.model.current_time.day == 1:
            if self.model.current_time.year - 1 > self.model.spinup_start.year:
                ...
                if "irrigation_limit" in self.config:
