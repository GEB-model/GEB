...
class ReservoirOperators(AgentBaseClass):
    ...
    def initiate_agents(self, waterBodyIDs):
        ...
        self.dis_avg = self.active_reservoirs[
            "average_discharge"
        ].values  # average 1971-2000
        ...
        self.N = self.cpa.shape[
            0
        ]  # Number of reservoirs. All arrays will have this dimension: (N,)
        self.S_begin_yr = self.cpa.copy()
        # self.S_begin_yr = self.model.data.grid.reservoirStorageM3C
        self.Sini_resv = self.S_begin_yr.copy()
        self.alpha = np.full(
            (self.N,), 0.85
        )  # set all reservoirs to a capacity reduction factor of 0.85 (Biemans et al., 2011).
        self.mtifl = np.full(
            (self.N,), self.active_reservoirs["average_discharge"].values
        )
        ...
        self.res_ppose = np.full(
            (self.N,), 1
        )  # temporarlily set all reservoirs to irrigation reservoirs, because data is still lacking.
        ...
        if self.model.load_initial_data:
            folder = os.path.join(
                self.model.initial_conditions_folder, "reservoir_operators"
            )
            ...
            self.mtifl_month_20yrs = np.full(
                (self.N, 12), 1, dtype="f8"
            )  # Stores 20 year moving average of mean monthly inflow for every month for every reservoir.
            self.monthly_infl_20yrs = np.full(
                (self.N, 12, 20), np.nan, dtype="f8"
            )  # Stores mean inflow for every month for every reservoir for the last 20 years.

            # Set starting values for moving averages
            self.mtifl_month_20yrs *= (
                self.mtifl.reshape(-1, 1) * 0.2
            )  # Fill mean monthly inflow with 25% average inflow of each reservoir, taken from database.

        ### Initiate variables for monthly and yearly calculations. ###
        self.new_mtifl = np.zeros(
            (self.N,), dtype="f8"
        )  # Temporary new mean total annual inflow
        self.new_irrmean = np.zeros(
            (self.N,), dtype="f8"
        )  # Temporary new mean irrigation demand
        self.irrmean = np.full((self.N), 1)  # Start with irrmean
        self.irrmean = (
            self.mtifl * 0.3
        )  # Set initial mean irrigation demand to 30% of mean inflow.

        self.monthly_infl = np.zeros(
            (self.N,), dtype="f8"
        )  # Total inflow in that month, used to calculate mean inflow for that month.
        self.n_months_in_new_year = np.zeros(
            (self.N,), dtype="f8"
        )  # Number of months in new hydrological year for each reservoir.

        self.nt_y = np.zeros(
            (self.N,), dtype="f8"
        )  # number of routing timesteps in hydrological year for each reservoir
        self.nt_m = 0  # number of routing timesteps in a month

        self.total_irr_demand_area_yesterday = np.zeros(
            (self.N,), dtype="f8"
        ...
        self.irr_demand_monthly = 0
        self.Qout_monthly = 0
        self.Qin_monthly = 0
        self.potential_extraction_monthly = 0
        self.actual_abstraction_monthly = 0
    ...
    def new_reservoir_management(
        ...
        self.inflow = (
            inflow / 3600
        )  # * self.model.InvDtSec    # convert inflow from m3/hr to m3/s
        Rres = self.inflow.copy()  # if there are no reservoirs, inflow equals outflow.
        Qin_resv = (
            self.inflow.copy()
        )  # make local copy of inflow, to calculate reservoir release.
        mtifl_month = self.mtifl_month_20yrs[:, self.date.month - 1]
        ...
        self.irr_demand = (
            self.model.data.HRU.reservoir_abstraction_m3.copy() * self.model.InvDtSec
        )

        # Make sure all variables are in same size and shape:
        assert (
            self.inflow.size == self.irr_demand.size == self.cpa.size
        ), "Variables for reservoir management module are not same size"
        assert (
            self.inflow.shape == self.irr_demand.shape == self.cpa.shape
        ), "Variables for reservoir management module are not same shape"
        ...
        if np.any(self.condI):
            Rres[self.condI] = self.irrigation_reservoir(
                self.cpa,
                self.condI,
                Qin_resv,
                self.S_begin_yr,
        ...
        if np.any(self.condF):
            Rres[self.condF] = self.flood_control_reservoir(
                self.cpa, self.condF, Qin_resv, self.S_begin_yr, self.mtifl, self.alpha
            )

        # Ensure environmental flow requirements, no reservoir overflow and no negative storage.
        S_ending = (
            Sini_resv.copy()
        )  # create temporary final storage variable. If nothing changes, initial storage will be final storage.
        ...
        # Get potential abstraction for the next day.
        self.release_for_abstraction += (
            Qout_resv - mtifl_month * 0.15
        )  # This is given to water_demand

        # Ensure that this cannot be negative
        self.release_for_abstraction = np.maximum(self.release_for_abstraction, 0)

        # Subtract actual abstraction from outflow today (to prevent double counting in storage subtraction)
        ...
        if np.any(Qout_resv < mtifl_month_min):
            # print(
            #     "Outflow is below minimum required outflow, due to abstraction previous day."
            # )
            Qout_resv[Qout_resv < mtifl_month_min] = mtifl_month_min[
                Qout_resv < mtifl_month_min
            ]

        # Sum all values to monthly values
        self.irr_demand_monthly += self.irr_demand
        self.Qout_monthly += Qout_resv
        self.Qin_monthly += self.inflow
        self.potential_extraction_monthly += self.release_for_abstraction / (
            NoRoutingExecuted if NoRoutingExecuted != 0 else 1
        )
        self.actual_abstraction_monthly += self.abstraction_prev_day_dt

        if NoRoutingExecuted == 0:

            if self.date.day == 1:
                # Print some information about the day.
                print(
                    "potential_extraction_monthly perc",
                    ((self.potential_extraction_monthly / self.cpa) * 100).round(2),
                    "Absolute:",
                    self.potential_extraction_monthly,
                )
                print(
                    "actual_abstraction_monthly perc",
                    ((self.actual_abstraction_monthly / self.cpa) * 100).round(2),
                    "Absolute:",
                    self.actual_abstraction_monthly,
                )
                print(
                    "irr_demand perc monthly is:",
                    ((self.irr_demand_monthly / self.cpa) * 100).round(2),
                    "Absolute:",
                    self.irr_demand_monthly,
                )
                print(
                    "Monthly inflow perc of reservoir is:",
                    ((self.Qin_monthly / self.cpa) * 100).round(2),
                    "Absolute:",
                    self.Qin_monthly,
                )
                print(
                    "Monthly outflow perc of reservoir is:",
                    ((self.Qout_monthly / self.cpa) * 100).round(2),
                    "Absolute:",
                    self.Qout_monthly,
                )
    ...
    def reservoir_water_balance(self, Qin, Qout, Sin, cpa, mtifl, alpha, cond_all):
        ...
        if Sa.any():
            ...
            Rspill = (
                Stemp[Sa] - (alpha[Sa] * cpa_[Sa])
            ) / dt  # Spilling is determined as earlier storage level minus maximum capacity.
            Rfinal[Sa] = (
                Qout_[Sa] + Rspill
        ...
        if Sb.any():
            ...
            Rfinal[Sb] = ((Stemp[Sb] - Sfinal[Sb]) / dt) + Qin_[
                Sb
    ...
    def determine_outflow_module(
        ...
        if "ruleset" in self.config and self.config["ruleset"] == "new_module":
            outflow, irr_demand = self.new_reservoir_management(
                inflow,
                NoRoutingExecuted,
                reservoirStorageM3C,
                pot_irrConsumption_m_per_cell,
        ...
        # Check if to use natural state, where inflow is outflow.
            ...
            "ruleset" in self.config and self.config["ruleset"] == "no-human-influence"
        ):
        ...
        # Otherwise, use original LISFLOOD reservoir module.
            ...
            outflow = self.regulate_reservoir_outflow(
                reservoirStorageM3C, inflow, waterBodyIDs
            )
            return outflow, None
    ...
    def get_irrigation_per_command_area(self, potential_irrigation_consumption_m):
        ...
        command_area_count = np.bincount(
            command_area_per_farmer[command_area_per_farmer != -1]
        )
    ...
    def _shift_and_reset_matrix_mask(
        self, matrix: np.ndarray, condition: np.ndarray = None
    ) -> None:
        """
