        ...
        for field_index, field in enumerate(farmer_fields):
            ...
            if crop_map[field] != -1:
                ...
                if use_cost_priority:
                    ...
                    BIG = np.float32(1e30)
                    c0 = (
                        np.float32(unit_cost_channel) if has_channel else BIG
                    )  # 0: channel
                    c1 = (
                        np.float32(unit_cost_reservoir) if has_reservoir else BIG
                    )  # 1: reservoir
                    c2 = (
                        np.float32(unit_cost_groundwater) if has_ground else BIG
                    )  # 2: groundwater
                    ...
                    for kk in range(3):
                        ...
                        # Select the remaining_limit array for the current water source (3 Channels)
                        if src == 0:
                            remaining_irrigation_limit_for_src = (
                                remaining_irrigation_limit_m3_channel
                            )
                        elif src == 1:
                            remaining_irrigation_limit_for_src = (
                                remaining_irrigation_limit_m3_reservoir
                            )
                        else:
                            remaining_irrigation_limit_for_src = (
                                remaining_irrigation_limit_m3_groundwater
                            )
                        ...
                        #     break
                        if remaining_irrigation_limit_for_src[farmer] <= 0.0:
                            remaining_irrigation_limit_for_src[farmer] = 0.0
                            continue

                        if src == 1:
                            # ===== Reservoir (1) =====
                            # 抽水前：用预算裁剪需水
                            irrigation_water_demand_field_m = _apply_budget_limit(
                                irrigation_water_demand_field_m,
                                unit_cost_reservoir,
                            )
