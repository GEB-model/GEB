...
def get_deficit_between_dates(
    cumulative_water_deficit_m3: npt.NDArray[np.floating],
    farmer: int | np.integer,
    start_index: int | np.integer,
    end_index: int | np.integer,
) -> np.float32:
    ...
    if end_index == start_index:
        ...
        deficit: np.float32 = cumulative_water_deficit_m3[farmer, -1] - (
...
def get_future_deficit(
    farmer: int | np.integer,
    day_index: int | np.integer,
    cumulative_water_deficit_m3: npt.NDArray[np.floating],
    crop_calendar: npt.NDArray[np.int32],
    crop_rotation_year_index: npt.NDArray[np.int32],
    potential_irrigation_consumption_farmer_m3: float | np.floating,
    reset_day_index: int | np.integer,
) -> np.float32:
    ...
    future_water_deficit: np.float32 = np.float32(
...
def adjust_irrigation_to_limit(
    ...
    farmer_gross_irrigation_demand_m3: float | np.floating,
    irrigation_efficiency_farmer: float | np.floating,
    reset_day_index: int | np.integer,
) -> np.float32:
    ...
    if np.all(~np.isnan(cumulative_water_deficit_m3[farmer])):
        ...
        irrigation_correction_factor: np.float32 = (
        ...
        irrigation_correction_factor: np.float32 = min(
            irrigation_correction_factor, np.float32(1)
        )
    else:
        irrigation_correction_factor: np.float32 = np.float32(1)
...
def withdraw_channel(
    available_channel_storage_m3: npt.NDArray[np.floating],
    grid_cell: int | np.integer,
    cell_area: npt.NDArray[np.floating],
    field: int | np.integer,
    farmer: int | np.integer,
    irrigation_water_demand_field_m: float | np.floating,
    water_withdrawal_m: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3: npt.NDArray[np.floating],
    channel_abstraction_m3_by_farmer: npt.NDArray[np.floating],
    maximum_abstraction_channel_m3_field: float | np.floating,
    minimum_channel_storage_m3: float | np.floating = 100.0,
) -> np.float32:
    ...
    irrigation_water_demand_field_m = max(irrigation_water_demand_field_m, 0.0)

    return np.float32(irrigation_water_demand_field_m)


@njit(cache=True)
def withdraw_reservoir(
    command_area: int | np.integer,
    field: int | np.integer,
    farmer: int | np.integer,
    reservoir_abstraction_m3: npt.NDArray[np.floating],
    available_reservoir_storage_m3: npt.NDArray[np.floating],
    irrigation_water_demand_field_m: float | np.floating,
    water_withdrawal_m: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3: npt.NDArray[np.floating],
    reservoir_abstraction_m3_by_farmer: npt.NDArray[np.floating],
    maximum_abstraction_reservoir_m3_field: float | np.floating,
    cell_area: npt.NDArray[np.floating],
) -> np.float32:
...
def withdraw_groundwater(
    ...
    grid_cell: int | np.integer,
    field: int | np.integer,
    groundwater_abstraction_m3: npt.NDArray[np.floating],
    available_groundwater_m3: npt.NDArray[np.floating],
    cell_area: npt.NDArray[np.floating],
    groundwater_depth: npt.NDArray[np.floating],
    well_depth: npt.NDArray[np.floating],
    irrigation_water_demand_field_m: float | np.floating,
    water_withdrawal_m: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3: npt.NDArray[np.floating],
    groundwater_abstraction_m3_by_farmer: npt.NDArray[np.floating],
    maximum_abstraction_groundwater_m3_field: float | np.floating,
) -> np.float32:
...
def get_potential_irrigation_consumption_m(
    ...
    soil_layer_height: ArrayFloat32,
    field_capacity: ArrayFloat32,
    wilting_point: ArrayFloat32,
    w: ArrayFloat32,
    ws: ArrayFloat32,
    saturated_hydraulic_conductivity_m_per_day: ArrayFloat32,
    fraction_irrigated_field: np.float32,
    max_paddy_water_level_farmer: np.float32,
    crop_group: np.float32,  # kept for future dynamic-p logic
    is_paddy: np.bool_,
...
def get_gross_irrigation_demand_m3(
    day_index: int,
    n: int,
    currently_irrigated_fields: ArrayBool,
    field_indices_by_farmer: TwoDArrayInt32,
    field_indices: ArrayInt32,
    irrigation_efficiency: ArrayFloat32,
    fraction_irrigated_field: ArrayFloat32,
    cell_area: ArrayFloat32,
    crop_map: ArrayInt32,
    topwater: ArrayFloat32,
    root_depth_m: ArrayFloat32,
    soil_layer_height: TwoDArrayFloat32,
    field_capacity: TwoDArrayFloat32,
    wilting_point: TwoDArrayFloat32,
    w: TwoDArrayFloat32,
    ws: TwoDArrayFloat32,
    saturated_hydraulic_conductivity_m_per_day: TwoDArrayFloat32,
    remaining_irrigation_limit_m3_reservoir: ArrayFloat32,
    remaining_irrigation_limit_m3_channel: ArrayFloat32,
    remaining_irrigation_limit_m3_groundwater: ArrayFloat32,
    irrigation_limit_reset_day_index: ArrayInt32,
    cumulative_water_deficit_m3: TwoDArrayFloat32,
    crop_calendar: ThreeDArrayInt32,
    crop_group_numbers: ArrayFloat32,
    paddy_irrigated_crops: ArrayBool,
    current_crop_calendar_rotation_year_index: ArrayInt32,
    max_paddy_water_level: ArrayFloat32,
    minimum_effective_root_depth_m: np.float32,
    ...
    gross_potential_irrigation_m3: ArrayFloat32 = np.zeros(
        n_hydrological_response_units, dtype=np.float32
    )
    gross_potential_irrigation_m3_limit_adjusted_reservoir: ArrayFloat32 = (
        np.zeros_like(gross_potential_irrigation_m3)
    )
    gross_potential_irrigation_m3_limit_adjusted_channel: ArrayFloat32 = np.zeros_like(
        gross_potential_irrigation_m3
    )
    gross_potential_irrigation_m3_limit_adjusted_groundwater: ArrayFloat32 = (
        np.zeros_like(gross_potential_irrigation_m3)
    )
    ...
    for farmer in range(n):
        ...
        if not np.isnan(remaining_irrigation_limit_m3_reservoir[farmer]):
            if farmer_gross_irrigation_demand_m3 > 0.0:
                irrigation_correction_factor: np.float32 = adjust_irrigation_to_limit(
                    farmer=farmer,
                    day_index=day_index,
...
def abstract_water(
    ...
    field_indices_by_farmer: npt.NDArray[np.int32],
    field_indices: npt.NDArray[np.int32],
    irrigation_efficiency: npt.NDArray[np.floating],
    ...
    return_fraction: npt.NDArray[np.floating],
    well_depth: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3_reservoir: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3_channel: npt.NDArray[np.floating],
    remaining_irrigation_limit_m3_groundwater: npt.NDArray[np.floating],
    maximum_abstraction_reservoir_m3_by_farmer: npt.NDArray[np.floating],
    maximum_abstraction_channel_m3_by_farmer: npt.NDArray[np.floating],
    maximum_abstraction_groundwater_m3_by_farmer: npt.NDArray[np.floating],
    gross_irrigation_demand_m3_per_field: npt.NDArray[np.floating],
) -> tuple[
    npt.NDArray[np.float32],
    npt.NDArray[np.float32],
    npt.NDArray[np.float32],
    ...
    for farmer in activation_order:
        ...
        for field_index, field in enumerate(farmer_fields):
            field_int: int = int(field)
            grid_cell: int = int(HRU_to_grid[field_int])
            grid_cell_nearest: int = int(nearest_river_grid_cell[field_int])
            if crop_map[field_int] != -1:
                irrigation_water_demand_field_m = (
                    gross_irrigation_demand_m3_per_field[field_int]
                    / cell_area[field_int]
                )
                ...
                if command_area_farmer != -1:  # -1 means no command area
                    ...
                    assert water_withdrawal_m[field_int] >= 0
...
@njit(cache=True)
def _pick_representative_by_mean_total_profit(
    total_profits: npt.NDArray[np.float32],  # (n_farmers, n_cols)
    candidate_mask: npt.NDArray[np.bool_],  # (n_farmers,)
    target_mean: np.float32,  # scalar
) -> np.int32:
    """Pick a donor farmer whose mean profit is closest to a target value.

    Compute the mean total profit across all columns for each candidate farmer
    and select the farmer whose mean is closest to the given target mean. Only
    farmers flagged as candidates are considered.

    Notes:
        The function ignores farmers not marked as candidates in the input mask.
        If no candidate yields an improvement over the initial distance, the
        default index remains -1.

    Args:
        total_profits: Profit matrix with one row per farmer and one column per
            profit scenario or event.
        candidate_mask: Boolean mask indicating which farmers are candidates for
            selection.
        target_mean: Target mean profit used as the reference for similarity.

    Returns:
        Index of the selected donor farmer in the first dimension of
        ``total_profits``. Returns -1 if no candidate is found.
    """
    n_farmers = total_profits.shape[0]
    n_cols = total_profits.shape[1]

    best_idx = np.int32(-1)
    best_dist = np.float32(1e20)
...
def crop_profit_difference_total(
    ...
    Notes:
        The function operates at group level, then maps group-level gains and
        donor indices back to individual farmers using the provided group index
        mapping. Gains are computed as candidate mean profit minus current mean
        profit for each column. Some combinations may have no valid donor group;
        in such cases, gains and donor indices remain at their initialized values.

    Args:
        total_profits: Profit matrix with one row per farmer and one column per
            event or scenario, including the no-event case.
        crop_elevation_group: Encoded crop-elevation information per farmer,
            used to assign farmers to unique crop groups.
        unique_crop_groups: Encoded representation of unique crop-elevation
            groups present in the population.
        group_indices: Mapping from farmer index to position in
            ``unique_crop_groups``, used to broadcast group-level results back
            to farmers.
        unique_crop_calendars: Encoded crop rotation calendars associated with
            each unique group and candidate rotation.

    Returns:
        A tuple containing:
            gains: Three-dimensional array with the profit gain for each farmer,
                candidate calendar, and scenario column. The shape is
                (number of farmers, number of calendars, number of columns).
            new_farmer_id: Two-dimensional array with the index of the
                representative donor farmer to switch to for each farmer and
                candidate calendar.
    """
    n_groups = unique_crop_groups.shape[0]
    n_calendars = unique_crop_calendars.shape[0]
    n_rotation = unique_crop_calendars.shape[1]
    n_cols = total_profits.shape[1]

    # group-level gains storage
    ...
    for group_id in prange(n_groups):  # ty:ignore[not-iterable]
        ...
        # candidate calendars: those whose rotation differs from current group's rotation
        candidate_idxs = np.empty(n_calendars, dtype=np.int32)
        num_cand = 0
        for cal in range(n_calendars):
            same_rotation = True
            for j in range(n_rotation):
                if unique_crop_calendars[cal, j] != unique_group[j]:
                    same_rotation = False
                    break
            if not same_rotation:
                candidate_idxs[num_cand] = cal
                num_cand += 1

        metadata = unique_group[n_rotation:]

        # current group mean vector across total_profits columns
        current_sum = np.zeros(n_cols, dtype=np.float32)
        count_cur = 0
        for i in range(unique_farmer_groups.size):
            if unique_farmer_groups[i]:
                for c in range(n_cols):
                    current_sum[c] += total_profits[i, c]
                count_cur += 1
        if count_cur == 0:
            continue
        current_mean = current_sum / count_cur

        # iterate candidate rotations
        for k in range(num_cand):
            cal_idx = candidate_idxs[k]
            unique_rotation = unique_crop_calendars[cal_idx]

            # build other key = candidate rotation + same metadata
            other_key = np.empty(unique_group.shape[0], unique_group.dtype)
            for r in range(n_rotation):
                other_key[r] = unique_rotation[r]
            for r in range(n_rotation, other_key.shape[0]):
                other_key[r] = metadata[r - n_rotation]

            unique_farmer_other = find_matching_rows(crop_elevation_group, other_key)
            if not np.any(unique_farmer_other):
                continue

            # candidate group mean vector
            cand_sum = np.zeros(n_cols, dtype=np.float32)
            count_cand = 0
            for i in range(unique_farmer_other.size):
                if unique_farmer_other[i]:
                    for c in range(n_cols):
                        cand_sum[c] += total_profits[i, c]
                    count_cand += 1
            if count_cand == 0:
                continue

            cand_mean = cand_sum / count_cand

            # store per-column gain vector
            for c in range(n_cols):
                unique_gain[group_id, cal_idx, c] = cand_mean[c] - current_mean[c]

            # representative donor: match mean across columns
            target_mean = np.float32(0.0)
            for c in range(n_cols):
                target_mean += cand_mean[c]
            target_mean /= n_cols

            id_to_switch_to[group_id, cal_idx] = (
                _pick_representative_by_mean_total_profit(
                    total_profits=total_profits,
                    candidate_mask=unique_farmer_other,
                    target_mean=target_mean,
                )
            )

    # re-index per farmer
    gains = unique_gain[group_indices, :, :]  # (n_farmers, n_calendars, n_cols)
    new_farmer_id = id_to_switch_to[group_indices, :]  # (n_farmers, n_calendars)

    return gains, new_farmer_id
...
def compute_premiums_and_best_contracts_numba(
    ...
    values_history: npt.NDArray[np.floating],
