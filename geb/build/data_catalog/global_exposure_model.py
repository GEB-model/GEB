...
matching_paths = [
    item["path"]
    for item in tree
    if item["type"] == "tree" and item["path"].split("/")[-1].lower() == COUNTRY.lower()
]
...
for item in tree:
    if item["type"] == "blob":
        for folder in matching_paths:
            if item["path"].startswith(folder + "/") and item["path"].lower().endswith(
                ".csv"
            ):
...
RAW_BASE = f"https://raw.githubusercontent.com/gem/global_exposure_model/{BRANCH}/"
