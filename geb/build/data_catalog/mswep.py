...
def _get_google_credentials() -> Credentials:
    ...
    if not client_id or not client_secret:
        raise FileNotFoundError(
            ...
            "GOOGLE_CLIENT_SECRET in your .env file or place a valid token in "
            "~/.config/geb/google_drive_token.json."
        )
...
def _download_and_extract_in_memory(
    ...
    Returns:
        A 2D xarray DataArray containing the precipitation data (mm/h).
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            bio: io.BytesIO = io.BytesIO(response.content)
            with xr.open_dataset(bio, engine="h5netcdf") as ds:
                precipitation: xr.DataArray = ds["precipitation"].compute()
                precipitation = precipitation.assign_coords(
                    time=pd.Timestamp(precipitation.time.item()).round("h")
                )
...
def _extract_spatial_coords(
    ...
    """Extract 1D y (latitude) and x (longitude) coordinate arrays from MSWEP xarray data.

    Args:
        data: The xarray DataArray or Dataset containing 'lat' and 'lon' coordinates.

    Returns:
        A tuple of (y_coords, x_coords) as 1D numpy float arrays (degrees).
    """
    y: np.ndarray = data.coords["lat"].values.astype(np.float64)
    x: np.ndarray = data.coords["lon"].values.astype(np.float64)
    return y, x
...
class MSWEPPrecipitation(Adapter):
    ...
    def _get_mask_and_coords(
        self,
        sample_file_id: str,
        creds: Credentials,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Load or create the 2D basin mask and extract 1D spatial coordinates.

        Args:
            sample_file_id: Google Drive file ID of an MSWEP NetCDF file for coordinate extraction.
            creds: Authenticated Google Credentials instance.

        Returns:
            A tuple of (y_coords, x_coords, mask_arr) where coordinates are in degrees.
        """
        if self.mask_path.exists():
            ...
            sample_da: xr.DataArray = _download_and_extract_in_memory(
                file_id=sample_file_id,
                session=session,
                creds=creds,
            )
            y_coords, x_coords = _extract_spatial_coords(sample_da)
        ...
        return (
            mask_da.y.values.astype(np.float64),
            mask_da.x.values.astype(np.float64),
            mask_da.values.astype(bool),
        )
    ...
    def fetch(
        ...
        **kwargs: Any,
        ...
        if self.mask_path.exists():
            mask_da: xr.DataArray = read_zarr(self.mask_path)
            y_coords = mask_da.y.values.astype(np.float64)
            x_coords = mask_da.x.values.astype(np.float64)
        ...
        for current_year in range(start_ts.year, raw_end_ts.year + 1):
            ...
            if mask_arr is None or y_coords is None or x_coords is None:
                y_coords, x_coords, mask_arr = self._get_mask_and_coords(
                    sample_file_id=all_year_files[0]["id"],
                    creds=creds,
                )
            ...
            for month in months_to_process:
                ...
                with (
                    ...
                    for future in concurrent.futures.as_completed(future_to_file):
                        da_slice: xr.DataArray = future.result()
                        t_val: pd.Timestamp = pd.Timestamp(  # ty:ignore[invalid-assignment]
                            da_slice.time.item()
                        )
                        idx: int | None = time_to_idx.get(t_val)
                        if idx is not None:
                            arr_2d: np.ndarray = da_slice.values.squeeze()
                            valid_mask: np.ndarray = mask_arr & ~np.isnan(arr_2d)
                            encoded_arr: np.ndarray = np.full_like(
