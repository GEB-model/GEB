        ...
        *,
        ...
        for path in tile_paths:
            ...
            if source_crs is None:
                da.close()
                raise ValueError(
                    f"WEkEO HRL source tile {path} has no CRS in its GeoTIFF metadata."
                )

            # 65535 is a documented Croplands ``Outside area`` code. Remove the
            # source GeoTIFF nodata metadata first and then, for model reads, map the
            # semantic outside code to the signed model sentinel *before* any GDAL
            # operation. This prevents GDAL from rewriting 65535 to 65534.
            da = self._clear_raster_nodata(da)
            if normalize_nodata:
            ...
            # Reassert metadata after clipping; xarray/rioxarray operations can
            # discard the grid-mapping encoding even when coordinates survive.
            da = da.rio.set_spatial_dims(x_dim="x", y_dim="y", inplace=False)
            da = da.rio.write_crs(source_crs, inplace=False)
        ...
        if len(das) == 1:
            # Regional reads commonly need only one 100 km tile. Returning its
            # clipped view avoids allocating a second, equally large mosaic.
            merged = das[0]
        else:
            try:
        ...
        if not normalize_nodata:
            # Raw inspection mode deliberately has no active nodata marker.
