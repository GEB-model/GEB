...
def calibrate(config, working_directory):
	...
	# water_use_data = get_observed_water_use(calibration_config)
	...
	for gauge in gauges:
		...
		# Drop rows with missing or invalid flow values
		df = df.dropna(subset=['Discharge (ML/Day)'])
		# Select and rename columns
		df = df[['Date', 'Discharge (ML/Day)']].rename(
			columns={'Date': 'date', 'Discharge (ML/Day)': 'flow'}
		)
		# Ensure 'flow' is numeric and non-negative
		df['flow'] = pd.to_numeric(df['flow'], errors='coerce')
		df = df[df['flow'] >= 0]
		# **Convert 'flow' from ML/day to m³/s**
		df['flow'] = df['flow'] * (1000 / 86400)  # Converts ML/day to m³/s
		# Format the 'date' column to 'YYYY-MM-DD' string format
		df['date'] = df['date'].dt.strftime('%Y-%m-%d')
		# Set 'date' as index to create a DatetimeIndex
		df = df.set_index(pd.to_datetime(df['date']))
		# Ensure the index is named 'time' to match simulated data
		df.index.name = 'time'
		# Store the 'flow' Series in the observed_streamflow dictionary
		observed_streamflow[gauge] = df['flow']
		# Set the name of the Series to 'observed' to match the format
