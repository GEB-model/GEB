...
def calculate_performance_metrics(
    ...
    output_folder: Path,
    run_name: str,
    elevation_data: Path | str,
    minimum_flood_depth: float,
    extra_validation_region: Path | None = None,
    ...
    Args:
        ...
        elevation_data: Path to the elevation data (.zarr format) for hillshade visualization.
        extra_validation_region: Optional path to an additional validation region for clipping the flood map.
    ...
    region = read_geom(
        Path("simulation_root") / run_name / "SFINCS" / "group_0" / "region.geoparquet"
    ).to_crs(obs.rio.crs)
    ...
    if extra_validation_region:
        extra_clip_region = gpd.read_file(extra_validation_region).set_crs(28992)
        ...
        clipped_out = (sim_no_rivers > minimum_flood_depth) & (
            sim_extra_clipped.isnull()
        )
    ...
    simulation_final = sim_extra_clipped > minimum_flood_depth
    ...
    false_rate = (
        calculate_false_alarm_ratio(sim_final_computed, obs_final_computed) * 100
    )
    csi = calculate_critical_success_index(sim_final_computed, obs_final_computed) * 100
    ...
    elevation_data: xr.DataArray = read_zarr(elevation_data)
    elevation_data = elevation_data.rio.reproject_match(obs)

    elevation_array = elevation_data.squeeze().astype("float32").compute().values
    ...
    if simulation_final.sum() > 0:
        ...
        if visualization_type == "OSM":
            ...
            ax.set_xlim(catchment_extent[0] - margin, catchment_extent[1] + margin)
            ax.set_ylim(catchment_extent[2] - margin, catchment_extent[3] + margin)
            ...
            simulation_filename = os.path.splitext(os.path.basename(flood_map_path))[0]
            fig.savefig(
                output_folder
            ...
            simulation_filename = os.path.splitext(os.path.basename(flood_map_path))[0]
            plt.savefig(
                output_folder / f"{simulation_filename}_validation_floodextent_plot.png"
            )
            print(
                f"Figure with {visualization_type} saved as: {output_folder / f'{simulation_filename}_validation_floodextent_plot.png'}"
            )
        ...
        for event in self.config["floods"]["events"]:
            ...
            if len(flood_map_files) == 1:
                ...
                calculate_performance_metrics(
                    ...
                    extra_validation_region=self.config["floods"].get(
                ...
                for forecast_init in unique_forecast_inits_list:
                    ...
                    for flood_map_path in matching_flood_maps:
                        ...
                        metrics = calculate_performance_metrics(
                            ...
                            elevation_data=self.config["floods"].get(
                                "elevation_data", None
                            ),
                        )
                        if metrics is None:
