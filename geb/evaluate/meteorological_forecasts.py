...
class MeteorologicalForecasts:
    ...
    def evaluate_forecasts(
        ...
        """Evaluate meteorological forecasts by comparing forecasts at different initialisation times (00:00 (not yet vs 12:00) UTC) 
        against forecast types such as ERA5 reanalysis data, ensemble forecasts and deterministic forecasts.
        ...
        Raises:
            ...
            FileNotFoundError: No precipitation zarr file found.
        """

        def evaluate_precipitation_forecasts(
                self, plot_type: str = 'intensity', *args: Any, **kwargs: Any
        ) -> None:
            ...
            Raises:
                ...
                FileNotFoundError: If no precipitation zarr file is found in initialization folder.
            """
            # Validate plot type
            if plot_type not in ["intensity", "cumulative"]:
                raise ValueError(f"plot_type must be 'intensity' or 'cumulative', got '{plot_type}'") 


            def load_forecast_data(
                init_folder: Path, plot_type: str = "intensity"
            ) -> tuple[xarray.DataArray, xarray.DataArray, np.ndarray, xarray.DataArray]:
                ...
                if not pr_files:
                    raise FileNotFoundError(f"No precipitation zarr file found in {init_folder}")
                
                if len(pr_files) > 1:
                    print(f"Warning: Multiple precipitation files found in {init_folder}, using first: {pr_files[0]}")
                ...
                ens_mm_per_h: xarray.DataArray = ens_ds * 3600  # Convert from m/s to mm/h
                ens_max_mm_per_h: xarray.DataArray = ens_mm_per_h.max(dim=["y", "x"])

                # ERA5 clip on time range control
                era5_clipped = era5_max_mm_per_h.sel(time=slice(ens_time[0], ens_time[-1]))

                # Apply cumulative sum if requested
                if plot_type == "cumulative":
                    era5_processed: xarray.DataArray = era5_clipped.cumsum(dim="time")
                    ens_processed: xarray.DataArray = ens_max_mm_per_h.cumsum(dim="time")
                else:
                ...
                ensemble_processed: xarray.DataArray = ens_processed.isel(member=slice(1, None))

                return era5_processed, control_processed, ens_time, ensemble_processed

            
            def format_time_axis(ax, x_start: pd.Timestamp, x_end: pd.Timestamp, x_ticks: list[pd.Timestamp]) -> None:
            ...
            # Plot the maximum rainfall intensity over time for each forecast initialization
            ...
            def plot_rainfall_data(
                ...
                Returns:
                    None
                """
                x_ticks: list[pd.Timestamp] = pd.date_range(start=x_start, end=x_end, freq='12h')
                
                # Parse initialization time string to readable format
                init_datetime: pd.Timestamp = pd.to_datetime(init_time_str, format='%Y%m%dT%H%M%S')
                init_time_readable: str = init_datetime.strftime('%d %B %Y, %H:%M UTC')
                # Plot ERA5 and control
                ax.plot(control_time, era5_data, color='black', label='ERA5', linewidth=2, alpha=0.8)
                ax.plot(control_time, control_data, color='green', label='Control', linewidth=2, alpha=0.8)
                ...
                moment_of_inundation: pd.Timestamp = pd.Timestamp('2024-05-06T00:00:00.000000000')
                ax.axvline(moment_of_inundation, color='red', linestyle='--', linewidth=2, label="Moment of Flooding Guaíba Lake")
                
                # Set title and formatting
                ax.set_title(f"Forecast Initialization: {init_time_readable}", fontsize=18)

                if plot_type == "cumulative":
                    ax.set_yticks(range(0, 1501, 250))  # Adjusted for cumulative mm values
                else:
            ...
            forecast_base_path: Path = self.model.input_folder / "other" / "forecasts" / "ECMWF" / "merged_control_ensemble"
            
            if not forecast_base_path.exists():
                raise ValueError(f"Forecast model path does not exist: {forecast_base_path}")
            ...
            num_cols: int = 2
            num_rows: int = (num_forecasts + num_cols - 1) // num_cols  # Ceiling division
            ...
            for idx, init_time in enumerate(forecast_initialisations):
                print(f"Processing forecast initialisation: {init_time}") # Log the iteration of forecasts
                ...
                if num_rows == 1:
                    ...
                    current_ax: plt.Axes = axes[row_idx, col_idx] if num_cols > 1 else axes[row_idx]
            ...
            for row in range(num_rows):
                for col in range(num_cols):
                    ...
                    if num_rows == 1:
                        ...
                        current_ax: plt.Axes = axes[row, col] if num_cols > 1 else axes[row]
                    ...
                    if idx >= num_forecasts:
                        ...
                        subplot_below_is_empty: bool = (row + 1) * num_cols + col >= num_forecasts
            ...
            # Save plot
            plt.savefig(forecast_folder / output_filename, dpi=1000, bbox_inches='tight')
            plt.close()  
        
        # Call the rainfall evaluation functions
        evaluate_precipitation_forecasts(self, plot_type="intensity")
        evaluate_precipitation_forecasts(self, plot_type="cumulative")