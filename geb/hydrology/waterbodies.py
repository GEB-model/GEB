...
class WaterBodies(Module):
    ...
    def spinup(self) -> None:
        ...
        waterbody_data: gpd.GeoDataFrame = self.load_waterbody_data(
    ...
    def step(self) -> None:
        ...
        if hasattr(self.var, "construction_year"):
            ...
            if not np.array_equal(active, self.is_active):
                river_storage_m3: ArrayFloat32 = (
                    self.hydrology.routing.router.get_total_storage(
                        self.grid.var.discharge_in_rivers_m3_s_substep,
                ...
                self.grid.var.discharge_in_rivers_m3_s_substep[new_reservoir_cells] = (
                    np.nan
                )
                self.hydrology.routing.router.waterbody_id = self.grid.var.waterbody_ids
                self.hydrology.routing.router.is_waterbody_outflow = (
                    self.grid.var.waterbody_outflow_points != -1
                )
