...
def init_multiple_fn(
    ...
    Args:
        ...
        region_shapefile: Shapefile to use for the region geometry. If the file does not exist, a bounding box geometry is created from geometry_bounds.
    ...
    if not region_shapefile:
        logger.info("Creating region shape from geometry shape...")
        bbox_geom = gpd.GeoDataFrame(
            geometry=[box(xmin, ymin, xmax, ymax)], crs="EPSG:4326"
        )
    else:
        logger.info("Loading existing region shape...")
