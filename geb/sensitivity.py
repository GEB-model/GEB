...
def sensitivity_parameters(parameters, distinct_samples, type = 'saltelli'):
    ...
    bounds = [[param_data['min'], param_data['max']] for param_data in parameters.values()]
...
def run_model(args):
    ...
    if runmodel:
        ...
        for i, parameter_data in enumerate(parameters.values()):
            individual_parameters[parameter_data['variable']] = param_adjustment_factor[i]
        ...
        while True:
            ...
            def run_model_scenario(scenario):
                ...
                if p.returncode == 0:  # model has run successfully
                    with open(os.path.join(logs_path, f"log{str(run_id)}_{scenario}.txt"), 'w') as f:
                        content = "OUTPUT:\n" + str(output.decode())+"\nERRORS:\n" + str(errors.decode())
                        f.write(content)
                    ...
                    with open(os.path.join(logs_path, f"log{str(run_id)}_{scenario}_{''.join((random.choice(string.ascii_lowercase) for x in range(10)))}.txt"), 'w') as f:
                        content = "OUTPUT:\n" + str(output.decode())+"\nERRORS:\n" + str(errors.decode())
                        f.write(content)
                    shutil.rmtree(run_directory)
...
def sensitivity_analysis(config, working_directory):
    ...
    param_adjustment_factor = sensitivity_parameters(parameters, distinct_samples, SA_type)

    pool_size = int(os.getenv('SLURM_CPUS_PER_TASK') or sensitivity_config['multiprocessing']['pool'])
    print(f'Pool size: {pool_size}')
    # Ignore the interrupt signal
    signal.signal(signal.SIGINT, signal.SIG_IGN)

    # Create a tuple of arguments for each run
    args = [(config, parameters, param_adjustment_factor[i], i) for i in range(len(param_adjustment_factor))]
    ...
    with multiprocessing.Pool(processes=pool_size, initializer=init_pool, initargs=(
            current_gpu_use_count, manager_lock, sensitivity_config['gpus'], sensitivity_config['models_per_gpu'] if 'models_per_gpu' in sensitivity_config else 1)) as pool:
