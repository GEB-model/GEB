...
class Plot:
    def __init__(self):
        self.fields = np.load('report/fields.npy')
        self.field_indices, self.fields_per_farmer = _set_fields(self.fields)
...
def read_irrigation_data(scenario):
    ...
    for i in range(timesteps):
        if i == 0:
            channel_irrigation = read_npy('channel irrigation', dt, scenario=scenario)
            groundwater_irrigation = read_npy('groundwater irrigation', dt, scenario=scenario)
            reservoir_irrigation = read_npy('reservoir irrigation', dt, scenario=scenario)
        else:
            channel_irrigation += read_npy('channel irrigation', dt, scenario=scenario)
            groundwater_irrigation += read_npy('groundwater irrigation', dt, scenario=scenario)
            reservoir_irrigation += read_npy('reservoir irrigation', dt, scenario=scenario)
    ...
    activation_order = np.load(os.path.join('report', scenario, 'activation order', 'initial.npy'))
    return channel_irrigation, groundwater_irrigation, reservoir_irrigation, activation_order
...
def plot_irrigation():
    ...
    channel_irrigation, groundwater_irrigation, reservoir_irrigation, activation_order = read_irrigation_data('base')
    ...
    plt.savefig('plot/output/irrigation_per_source.png')
    plt.savefig('plot/output/irrigation_per_source.svg')
    # plt.show()
