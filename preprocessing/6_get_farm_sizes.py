...
def submitForm(driver, state: str, district: str, downloadDir: str) -> None:
    ...
    """
    new_file = os.path.join(downloadDir, state + '-' + district  + '.csv')
    ...
    old_file = os.path.join(downloadDir, 'DistTableDisplay3.csv')
...
def export_farm_sizes(farm_size_shapefile, root_dir):
    ...
    with rasterio.open(os.path.join(root_dir, 'farmsize.tif'), 'w', **profile) as dst:
    ...
    with open(os.path.join(root_dir, 'farmsize.txt'), 'w') as f:
        f.write("\n".join(SIZE_CLASSES))
