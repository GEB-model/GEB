...
def main():
    ...
    with rasterio.open(os.path.join(INPUT, 'landsurface', 'full_region_cultivated_land.tif'), 'r') as src:
        cultivated_land = src.read(1)

    with rasterio.open(os.path.join(INPUT, 'areamaps', 'region_subgrid.tif'), 'r') as src:
        regions_grid = src.read(1)

    with rasterio.open(os.path.join(INPUT, 'areamaps', 'region_cell_area_subgrid.tif'), 'r') as src:
        cell_area = src.read(1)

    regions_shapes = gpd.read_file(os.path.join(INPUT, 'areamaps', 'regions.geojson'))

    farm_sizes_per_country = data_catalog.get_dataframe('lowder_farm_sizes').dropna(subset=['Total'], axis=0).drop(['empty', 'income class'], axis=1)
    ...
    for _, region in regions_shapes.iterrows():
        ...
        total_cultivated_land_area_lu = cell_area[(regions_grid == UID) & (cultivated_land == True)].sum()
        average_cell_area_region = cell_area[(regions_grid == UID) & (cultivated_land == True)].mean()

        country_farm_sizes = farm_sizes_per_country.loc[(farm_sizes_per_country['Country'] == country)].drop(['Country', "Census Year", "Total"], axis=1)
        assert len(country_farm_sizes) == 2, f'Found {len(country_farm_sizes) / 2} country_farm_sizes for {country}'
        
        n_holdings = country_farm_sizes.loc[country_farm_sizes['Holdings/ agricultural area'] == 'Holdings'].iloc[0].drop(['Holdings/ agricultural area']).replace('..', '0').astype(np.int64)
        agricultural_area_db_ha = country_farm_sizes.loc[country_farm_sizes['Holdings/ agricultural area'] == 'Agricultural area (Ha) '].iloc[0].drop(['Holdings/ agricultural area']).replace('..', '0').astype(np.int64)
