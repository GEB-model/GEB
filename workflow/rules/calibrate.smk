...
rule generate_flood_maps:
    ...
    run:
        # Copy the entire canonical `flood_maps` folder into the calibration
        # run directory. This copies all maps (not only configured return
        # periods) so individual runs can reference them. Keep diagnostic
        # logging and fail loudly if the source cannot be found.
        ...
        # Remove any previous copy then copy the entire folder
        try:
            if dst.exists():
                shutil.rmtree(dst)
            ...
            with open(log[0], "a") as fh:
                fh.write(f"Failed copying flood_maps: {e}\n")
            raise RuntimeError(f"Failed to copy flood_maps: {e}")
...
rule evaluate_individual:
    ...
    run:
        ...
        # Inline cleanup: remove only the per-run `output/flood_maps` to free space

        try:
            local_fm = run_dir / "output" / "flood_maps"
            with open(log[0], "a") as fh:
                fh.write(f"--- inline cleanup_flood_maps for {run_dir} ---\n")
                fh.write(f"local_fm.exists: {local_fm.exists()}, is_symlink: {local_fm.is_symlink()}\n")

            if local_fm.exists() or local_fm.is_symlink():
                try:
                    if local_fm.is_symlink():
                        local_fm.unlink()
                    elif local_fm.is_dir():
                        shutil.rmtree(local_fm)
                    else:
                        local_fm.unlink()
                except Exception as e:
                    with open(log[0], "a") as fh:
                        fh.write(f"Failed to remove local flood_maps: {e}\n")
        except Exception:
            pass

        # touch a done-file so the previous separate cleanup rule remains compatible
        Path(run_dir / "flood_maps_removed.done").touch()
